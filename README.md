...
# 生成指定月份的绩效统计（格式：YYYY-MM-DD，任意日期即可）
node generate-performance-stats.js month 2026-02-01

# 生成本周的绩效统计
node generate-performance-stats.js week

# 生成当天的绩效统计
node generate-performance-stats.js day
```
...
| `node generate-performance-stats.js` | 生成测试等级绩效统计报告 | `day`/`week`/`month` + 可选日期 |
...
基于工作记录中的**测试等级**字段，自动计算绩效分数。支持按天、周、月三种维度统计。
...
# 生成指定日期所在周的绩效统计
node generate-performance-stats.js week 2026-02-15
```
...
- **必需字段**:
  - 工作表名: `工作记录`
  - 列名: `登记人`、`登记日期`、`开发完成日期`、`类别`、`任务内容`、`产品标识`、`项目名称`
  - 绩效统计专用: `测试等级`（格式：A类、2A类、B类等，或简写为A、2A、B等）
...
**A: 检查以下几点**
1. 确认工作记录Excel中是否有"测试等级"列
2. 检查测试等级列中是否有数据
3. 确认测试等级格式是否正确（支持 `A`、`2A`、`A类`、`2A类` 等格式）
4. 运行时查看调试信息，确认是否正确识别测试等级
5. 示例：如果看到"记录1 - 测试等级原始值: 2E"但没有识别出来，说明格式匹配有问题

### Q: 如何正确填写测试等级？

**A: 支持以下格式**
- **标准格式**（推荐）：`A类`、`2A类`、`B类`、`2B类`、`C类`、`2C类`、`D类`、`2D类`、`E类`、`2E类`
- **简写格式**：`A`、`2A`、`B`、`2B`、`C`、`2C`、`D`、`2D`、`E`、`2E`
- **多个等级**：用逗号分隔，如 `2E,E` 或 `2E类,E类`

### Q: 绩效统计的日期参数如何使用？

**A: 使用说明**
- **月度统计**：只需指定月份中的任意一天即可，如 `2026-02-01` 会统计整个2月
- **周报统计**：指定某一天，会自动计算该天所在的周（周一到周日）
- **日报统计**：指定具体日期，只统计当天数据
